...
def softmax_loss(Z, y):
    ...
    return np.mean(np.log(np.sum(np.exp(Z), 1)) - Z[np.arange(Z.shape[0]), y])


def softmax(x):
    x_exp = np.exp(x)
    x_sum = np.sum(x_exp, 1)[:, np.newaxis]
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for _ in range(0, X.shape[0], batch):
        ...
        Z = softmax(np.matmul(X_batch, theta))
        I = np.eye(theta.shape[1])[y_batch]
        G = np.matmul(np.transpose(X_batch), Z - I) / batch
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for _ in range(0, X.shape[0], batch):
        ...
        Z1 = np.maximum(np.matmul(X_batch, W1), [0])
        I = np.eye(W2.shape[1])[y_batch]
        G2 = softmax(np.matmul(Z1, W2)) - I
        G1 = (Z1 > 0) * np.matmul(G2, np.transpose(W2))

        W1[:, :] -= lr / batch * np.matmul(np.transpose(X_batch), G1)
        W2[:, :] -= lr / batch * np.matmul(np.transpose(Z1), G2)
